...
// Akun guru hasil registrasi — dikenali lintas perangkat lewat NIP/NIK unik.
interface GuruAccountData {
  npsn: string;
  namaSekolah: string;
  nama: string;
  nip: string;
...
interface SchoolDataStore {
  ...
  accounts: Record<string, GuruAccountData>;
}
...
function loadSchoolData(): SchoolDataStore {
  ...
  return { logins: {}, guruRows: {}, accounts: {} };
}
...
async function startServer() {
  ...
  // ===== REGISTRASI AKUN GURU (lintas perangkat, kunci = NIP/NIK) =====
  // Guru menyimpan akunnya ke server saat registrasi agar tetap dikenali
  // oleh NIP/NIK di perangkat/browser lain (tanpa registrasi ulang).
  app.post("/api/guru-register", (req, res) => {
    try {
      const { npsn, namaSekolah, nama, nip, alamat, kota, fase, kelas, isDemo } = req.body || {};
      const cleanNip = String(nip || "").trim().replace(/\s+/g, "");
      if (!cleanNip || !npsn || !namaSekolah || !nama) {
        return res.status(400).json({ error: "Parameter nip, npsn, namaSekolah, dan nama wajib." });
      }
      const store = loadSchoolData();
      store.accounts[cleanNip] = {
      ...
      return res.status(500).json({ error: err.message || "Gagal menyimpan akun guru." });
    }
  });

  // Cek akun guru berdasarkan NIP/NIK: GET /api/guru-register?nip=...
  // Tanpa parameter -> kembalikan daftar semua akun (untuk kebutuhan kepsek).
  app.get("/api/guru-register", (req, res) => {
    try {
      const nip = String(req.query.nip || "").trim().replace(/\s+/g, "");
      const store = loadSchoolData();
      if (nip) {
        return res.json({ account: store.accounts[nip] || null });
      }
      return res.json({ accounts: Object.values(store.accounts) });
    } catch (err: any) {
      return res.status(500).json({ error: err.message || "Gagal mengambil akun guru." });
    }
  });
