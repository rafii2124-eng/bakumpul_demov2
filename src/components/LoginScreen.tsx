...
import { pushGuruLogin, pushGuruRegistration, fetchGuruAccount } from '../utils/schoolSync';
...
export interface UserAccount {
  ...
  kelas?: string;
  isDemo?: boolean;
  /** Akun demo yang diizinkan mengolah data (input/editing) dari menu Diagnosa s.d. Jurnal Mengajar */
  demoEditData?: boolean;
}
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess, initialBiodata }) => {
  ...
  const getSavedCreds = (): { username?: string; password?: string } => {
    try {
      const raw = localStorage.getItem('bakumpul_saved_creds');
      if (raw) {
  ...
  const [rememberMe, setRememberMe] = useState<boolean>(() => {
    try {
      return localStorage.getItem('bakumpul_remember_me') === 'true';
    } catch {
      return false;
    }
  });

  const [usernameInput, setUsernameInput] = useState<string>(() => getSavedCreds().username?.trim() || '30301234');
  const [passwordInput, setPasswordInput] = useState<string>(() => getSavedCreds().password || '19850101 201001 1 001');
  ...
  // Akun guru yang sudah registrasi bersifat PERSISTEN: tidak dihapus saat
  // layar login dibuka ulang, sehingga NPSN & NIP yang terdaftar tetap
  // dikenali sistem meski guru keluar masuk aplikasi berkali-kali.
  // NPSN atau NIP yang tidak terdaftar tetap ditolak oleh handleLoginSubmit.
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    // Jika NIP/NIK tidak ditemukan di perangkat ini, cek ke server.
    // Dengan ini guru yang sudah registrasi TETAP dikenali oleh NIP/NIK
    // meski login dari browser/perangkat lain (tanpa registrasi ulang).
    if (!matchedAccount && cleanPass) {
      const serverAcc = await fetchGuruAccount(cleanPass);
      if (serverAcc) {
        const demoAcc = DEFAULT_ACCOUNTS.find((d) => cleanNipOf(d.nipGuru) === cleanNipOf(serverAcc.nip));
        matchedAccount = {
          npsn: serverAcc.npsn,
          namaSekolah: serverAcc.namaSekolah,
          namaGuru: serverAcc.nama,
          nipGuru: serverAcc.nip,
          alamat: serverAcc.alamat,
          kota: serverAcc.kota,
          fase: serverAcc.fase,
          kelas: serverAcc.kelas,
          isDemo: serverAcc.isDemo,
          demoEditData: demoAcc?.demoEditData
        };
        // Cache akun ke perangkat ini agar login berikutnya cepat (offline-ready)
        const cached = [matchedAccount, ...accounts.filter((a) => cleanNipOf(a.nipGuru) !== cleanNipOf(matchedAccount!.nipGuru))];
        setAccounts(cached);
        try {
          localStorage.setItem('bakumpul_accounts', JSON.stringify(cached));
        } catch {
          // fallback
        }
      }
    ...
    if (matched) {
      ...
      const schoolOk =
        cleanUser === matched.npsn.trim().toLowerCase() ||
        cleanUser === matched.namaSekolah.trim().toLowerCase() ||
        matched.npsn.trim().toLowerCase() === cleanUser ||
        accounts.some(a => cleanNipOf(a.nipGuru) === cleanNipOf(matched.nipGuru) && a.npsn.trim().toLowerCase() === cleanUser);
      if (schoolOk) {
        const isDemoAcc = matched.isDemo ?? (DEFAULT_ACCOUNTS.some(d => d.npsn === matched.npsn));
        markOnline(matched);
        persistRememberedLogin();
        onLoginSuccess({ ...matched, isDemo: isDemoAcc }, selectedSemester);
        return;
      }
    }
  ...
  // Masuk langsung dengan akun demo: tandai online, ingat login, lalu buka aplikasi
  const loginAsDemo = (acc: UserAccount) => {
    setUsernameInput(acc.npsn);
    setPasswordInput(acc.nipGuru);
    setSelectedSchool(acc.namaSekolah);
    markOnline({ ...acc, isDemo: true });
    persistRememberedLogin();
    onLoginSuccess({ ...acc, isDemo: true }, selectedSemester);
  };
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    // Setiap guru unik berdasarkan NIP/NIK (bukan NPSN) agar beberapa guru
    // di sekolah yang sama tidak saling menimpa akun.
    const updated = [newAccount, ...accounts.filter(a => cleanNipOf(a.nipGuru) !== cleanNipOf(newAccount.nipGuru))];
    setAccounts(updated);
    try {
    ...
    // Kirim akun ke server agar dikenali lintas perangkat/browser oleh NIP/NIK
    // (best-effort: gagal server tidak menggagalkan registrasi lokal).
    pushGuruRegistration(newAccount);
    ...
    setRegSuccessMsg(
      'Registrasi Berhasil! Akun tersimpan di perangkat & server. NIP/NIK Anda kini dikenali sistem meski login dari browser/perangkat lain. Anda bisa langsung Masuk.'
    );
    ...
    <div className="min-h-screen bg-slate-900 text-slate-800 font-sans flex flex-col justify-between selection:bg-indigo-600 selection:text-white">
      ...
      <header className="bg-indigo-900 text-white border-b border-indigo-800 px-4 sm:px-8 py-3 flex flex-wrap items-center justify-between shadow-lg z-20">
        ...
        <div className="flex items-center space-x-3 sm:space-x-5 text-xs font-bold text-indigo-100">
          ...
          <button
            disabled
            title="Registrasi dikunci"
            className="flex items-center space-x-1.5 transition bg-white/10 px-2.5 py-1.5 rounded-lg border border-white/15 cursor-not-allowed opacity-50"
          >
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-3 sm:p-6 lg:p-8 flex items-center justify-center">
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-0 lg:gap-6 bg-white rounded-3xl shadow-2xl overflow-hidden border border-indigo-900/30">
            ...
            <div className="my-4 flex flex-col items-center justify-center text-center relative z-10 space-y-4">
              <div className="pt-2">
                ...
                <p className="text-xs text-indigo-200 mt-1 max-w-md mx-auto">
                  Sistem Informasi Asesmen Diagnosa, KKTP, Buku Nilai, Presensi, dan Jurnal Harian Mengajar SD
                </p>
              </div>
            </div>
          ...
          <div className="lg:col-span-5 bg-white p-6 sm:p-8 flex flex-col justify-center">
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <div className="flex rounded-lg border border-slate-300 overflow-hidden shadow-xs focus-within:border-indigo-800 focus-within:ring-1 focus-within:ring-indigo-800 transition">
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 text-xs text-slate-900 font-medium focus:outline-none bg-white cursor-text"
                />
              </div>
              ...
              <div className="flex rounded-lg border border-slate-300 overflow-hidden shadow-xs focus-within:border-indigo-800 focus-within:ring-1 focus-within:ring-indigo-800 transition relative">
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 pr-10 text-xs text-slate-900 font-medium focus:outline-none bg-white cursor-text"
                />
                <button
                  type="button"
            ...
            {/* DEMO ACCOUNT LIST */}
            {DEFAULT_ACCOUNTS.filter((a) => a.isDemo).length > 0 && (
              <div className="mt-4 rounded-xl border border-dashed border-amber-400 bg-amber-50/70 p-3 space-y-2">
                <p className="text-[11px] font-black text-amber-800 uppercase tracking-wider flex items-center space-x-1.5">
                  <KeyRound className="w-3.5 h-3.5 text-amber-600" />
                  <span>Akun Demo</span>
                  <span className="text-[9px] font-bold text-amber-700/70 normal-case tracking-normal">(klik untuk masuk langsung)</span>
                </p>
                {DEFAULT_ACCOUNTS.filter((a) => a.isDemo).map((acc) => (
                  <div key={acc.nipGuru} className="bg-white rounded-lg border border-amber-200 p-2.5 space-y-1">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs font-bold text-slate-800 truncate">{acc.namaGuru}</p>
                      <span className="text-[9px] font-black text-white bg-amber-500 px-1.5 py-0.5 rounded-full uppercase tracking-wider shrink-0">
                        Demo
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-600 font-medium">
                      NPSN: <span className="font-bold text-indigo-900">{acc.npsn}</span>
                      <span className="mx-1 text-slate-300">|</span>
                      Sekolah: <span className="font-bold text-slate-800">{acc.namaSekolah}</span>
                    </p>
                    <p className="text-[11px] text-slate-600 font-medium">
                      NIP/NIK: <span className="font-bold text-indigo-900">{acc.nipGuru}</span>
                    </p>
                    <button
                      type="button"
                      onClick={() => loginAsDemo(acc)}
                      className="w-full mt-1 bg-amber-100 hover:bg-amber-200 text-amber-900 font-bold text-[11px] py-1.5 rounded-lg transition cursor-pointer"
                    >
                      Pakai Akun Demo Ini
                    </button>
                  </div>
            ...
            <div className="space-y-3 text-xs text-slate-700 leading-relaxed max-h-80 overflow-y-auto pr-1">
              ...
              <div className="bg-blue-50 p-3 rounded-xl border border-blue-200">
                ...
                <ul className="list-disc list-inside space-y-1">
                  ...
                  <li><strong>Presensi & Jurnal Mengajar:</strong> Pencatatan kehadiran dan jurnal harian 1 lembar PDF.</li>
                </ul>
              </div>
            </div>
