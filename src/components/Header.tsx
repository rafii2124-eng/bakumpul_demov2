...
interface HeaderProps {
  ...
  isDemo?: boolean;
  demoDataEdit?: boolean;
}

const NAV_ITEMS: { id: MainTab; label: string; icon: React.FC<{ className?: string }>; badge?: string }[] = [
  ...
  { id: 'atp', label: 'Generator ATP', icon: Workflow, badge: 'Baru' },
    ...
    <header className="bg-indigo-900 text-white shadow-md no-print sticky top-0 z-50">
          ...
          <div className="max-w-7xl mx-auto w-full flex flex-wrap items-center justify-between gap-1">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-semibold text-slate-900 text-[11px] sm:text-xs">
                {demoDataEdit
                  ? 'Anda dapat mengolah data dari menu Asesmen Diagnosa s.d. Jurnal Mengajar. Menu Biodata Sekolah dan Cetak/PDF tetap dikunci.'
            ...
            <span className="text-[10px] font-black text-indigo-950 bg-amber-200/90 px-2 py-0.5 rounded border border-amber-600/40 flex items-center space-x-1 shrink-0">
              ...
              <span>{demoDataEdit ? 'Dapat Mengolah Data' : 'Hanya Lihat (Read-Only)'}</span>
            </span>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo & Title */}
          <div className="flex items-center space-x-2.5 sm:space-x-3 mr-2 shrink-0">
            ...
            <div>
              <h1 id="header-web-title" className="text-base sm:text-lg font-extrabold tracking-wider text-white uppercase leading-none">
                BAKUMPUL
              </h1>
            </div>
          </div>
          ...
          <nav className="hidden lg:flex items-center space-x-1 overflow-x-auto max-w-[56rem] shrink-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`whitespace-nowrap px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 flex items-center justify-center space-x-1.5 cursor-pointer ${
                ...
                >
                  ...
                  {item.badge && <span className="bg-amber-300 text-indigo-950 text-[8px] uppercase font-black px-1 py-0.5 rounded shrink-0">{item.badge}</span>}
                </button>
              );
            })}
          </nav>
          ...
          <div className="flex items-center space-x-2 ml-auto shrink-0">
            {currentUser && (
              <div className="hidden sm:flex flex-col items-end text-right mr-2 text-[11px]">
            ...
            {onLogout && (
              <button
                ...
                className="bg-red-800/90 hover:bg-red-700 text-white border border-red-700/80 px-3 py-1.5 rounded-xl text-xs font-bold flex items-center space-x-1.5 transition shadow-sm cursor-pointer active:scale-95"
              >
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => handleSelectTab(item.id)}
                  className={`w-full text-left px-3.5 py-2.5 rounded-xl text-xs font-extrabold flex items-center justify-between transition-all whitespace-nowrap ${
                ...
                >
                  ...
                  {item.badge && <span className="bg-amber-300 text-indigo-950 text-[8px] uppercase font-black px-1 py-0.5 rounded shrink-0 ml-2">{item.badge}</span>}
                </button>
              );
            })}
          </div>
