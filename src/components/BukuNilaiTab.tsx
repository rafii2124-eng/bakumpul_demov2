...
export const BukuNilaiTab: React.FC<BukuNilaiTabProps> = ({
  ...
  const deleteStudent = (studentId?: number | string, indexFallback?: number) => {
    ...
    showToast("Data siswa berhasil dihapus dari Buku Nilai & Presensi.");
  };
  ...
  const addStudentToBukuNilai = () => {
    ...
    showToast("Siswa baru berhasil ditambahkan ke Buku Nilai & Presensi.");
  };
    ...
    <div id="tab-bukunilai" className="tab-content space-y-6">
      ...
      <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200 flex flex-wrap gap-1 no-print">
        <button
          ...
          className={`whitespace-nowrap px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition flex items-center space-x-2 cursor-pointer ${
        ...
        <div id="bn-subview-rekap-matpel" className="bn-subview space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-200 print-card space-y-4">
            <div className="overflow-x-auto">
              <table className="w-full border-2 border-slate-800 text-[10px] text-slate-900 font-medium table-fixed">
                <thead>
                  <tr className="bg-indigo-900 text-white border-b-2 border-slate-800">
                    <th rowSpan={2} className="border-r border-slate-700 px-1 py-2 text-center w-[3%]">NO</th>
                    <th rowSpan={2} className="border-r border-slate-700 px-1 py-2 text-center w-[7%]">NIS / NISN</th>
                    <th rowSpan={2} className="border-r border-slate-700 px-2 py-2 text-left w-[13%]">NAMA PESERTA DIDIK</th>
                    <th rowSpan={2} className="border-r border-slate-700 px-1 py-2 text-center w-[4%]">KELAS</th>
                    <th colSpan={getLmCount(rekapMatpelSubject)} className="border-r border-slate-700 px-2 py-1 text-center bg-indigo-950 w-[24%]">
                      SUMATIF LINGKUP MATERI (LM)
                    </th>
                    <th rowSpan={2} className="border-r border-slate-700 px-1 py-2 text-center w-[4%] bg-indigo-950 font-extrabold">RATA LM</th>
                    <th rowSpan={2} className="border-r border-slate-700 px-1 py-2 text-center w-[4%] bg-amber-500 text-indigo-950 font-extrabold">SAS</th>
                    <th rowSpan={2} className="border-r border-slate-700 px-1 py-2 text-center w-[4%] bg-emerald-700 font-black text-xs">NA RAPOR</th>
                    <th rowSpan={2} className="border-r border-slate-700 px-1 py-2 text-center w-[8%] bg-indigo-800 font-extrabold">KETUNTASAN</th>
                    <th rowSpan={2} className="border-r border-slate-700 px-2 py-2 text-left w-[26%]">CAPAIAN KOMPETENSI (DESKRIPSI RAPOR)</th>
                    <th rowSpan={2} className="px-1 py-2 text-center w-[3%] no-print">AKSI</th>
                  </tr>
                  <tr className="bg-indigo-800 text-indigo-100 border-b-2 border-slate-800">
                    {Array.from({ length: getLmCount(rekapMatpelSubject) }).map((_, i) => (
                      <th key={i} className="border-r border-slate-700 px-1 py-0.5 text-center text-[9px]" title={`Lingkup Materi ${i + 1}`}>
                        LM {i + 1}
                      </th>
                    ))}
                  </tr>
                  ...
                  {filteredRows.map((r, idx) => {
                    ...
                    return (
                      <tr key={s.id || idx} className={`border-b border-slate-300 hover:bg-slate-50 ${rowColor}`}>
                        <td className="text-center font-bold px-1 py-1 border-r border-slate-300">{idx + 1}</td>
                        <td className="text-center px-1 py-1 border-r border-slate-300">
                          <input
                            type="text"
                            value={s.nisn}
                            onChange={(e) => updateRekapGrade(rekapMatpelSubject, s.id, 'nisn', e.target.value)}
                            className="w-full text-center text-slate-600 bg-transparent focus:bg-white focus:outline-none text-[10px]"
                          />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-300">
                          <input
                            ...
                            className="w-full font-bold text-slate-800 bg-transparent focus:bg-white focus:outline-none text-[10px]"
                          />
                        </td>
                        <td className="text-center px-1 py-1 border-r border-slate-300 font-bold text-slate-700">
                          {biodata.kelas}
                        </td>

                        {lms.map((val, lmIdx) => (
                          <td key={lmIdx} className="text-center px-1 py-1 border-r border-slate-300">
                            <input
                              type="number"
                              min="0"
                              max="100"
                              ...
                              className="w-full text-center bg-transparent focus:bg-white focus:outline-none font-semibold text-[10px]"
                            />
                          </td>
                        ))}

                        <td className="text-center px-1 py-1 border-r border-slate-300 font-bold bg-indigo-50 text-indigo-950">
                          {avgLM}
                        </td>
                        <td className="text-center px-1 py-1 border-r border-slate-300">
                          <input
                            ...
                            className="w-full text-center bg-amber-50 font-bold text-slate-900 focus:bg-white focus:outline-none text-[10px]"
                          />
                        </td>
                        <td className="text-center px-1 py-1 border-r border-slate-300 font-black text-xs bg-emerald-100 text-emerald-950">
                          {na}
                        </td>
                        <td className="text-center px-1 py-1 border-r border-slate-300">
                          <span className={`inline-block whitespace-nowrap px-1.5 py-0.5 rounded-full text-[9px] font-black border ${badge}`}>
                            {badgeLabel}
                          </span>
                        </td>
                        <td className="px-2 py-1 border-r border-slate-300 text-[10px] leading-snug text-slate-700">
                          <span className={`block text-[8px] font-black uppercase tracking-wide mb-0.5 ${levelColor}`}>
                            {level}
                          </span>
                          {deskripsi}
                        </td>
                        <td className="text-center px-1 py-1 no-print">
                          <button
                            type="button"
                            onClick={(e) => {
                              e.preventDefault();
        ...
        <div id="bn-subview-input-evaluasi" className="bn-subview space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 no-print space-y-4">
            ...
            <div className="bg-sky-50 border border-sky-200 rounded-xl p-3 text-[11px] text-sky-800 flex items-start space-x-2">
              ...
              <span>
                ...
                Data akan menggantikan daftar siswa untuk mapel aktif dan otomatis tersinkron ke menu Presensi.
              </span>
            </div>
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-slate-200 print-card space-y-4">
            <div className="flex items-center justify-between no-print border-b pb-3">
              ...
              <p className="text-[11px] text-slate-400 italic">* Data siswa sama untuk semua mata pelajaran & tersinkron ke Presensi.</p>
            </div>
