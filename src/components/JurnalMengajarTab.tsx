...
// Ubah teks tujuan pembelajaran (mungkin multi-baris) menjadi daftar bernomor:
// "1. <desc>", "2. <desc>", dst.
const formatNumberedTps = (raw: string): string =>
...
export const JurnalMengajarTab: React.FC<JurnalMengajarTabProps> = ({
  ...
  // Helper to parse date string into Month Name and Day number for Presensi database lookup
  ...
  // Sync attendance data from Presensi database for a given date
  const getAttendanceFromAbsensi = (tanggalStr: string) => {
    ...
    if (totalRecorded === 0) {
      // Tidak ada catatan presensi untuk tanggal ini -> default semua hadir
      return {
      ...
      // Jumlah persis sama dengan tabel Presensi: hanya sel yang diisi H/S/I/A yang dihitung
      return {
        hadir: hadirCount,
        sakit: sakitCount,
        izin: izinCount,
  ...
  const [formData, setFormData] = useState<{
    ...
    return {
      ...
      tujuanPembelajaran: (rpmSource?.tps && rpmSource.tps.length > 0) ? rpmSource.tps.join('\n') : 'Peserta didik mampu memahami sejarah singkat dan makna lambang Garuda Pancasila.',
  ...
  const handleManualSyncAbsensi = () => {
    ...
    if (att) {
      ...
      showToast(`Kehadiran berhasil disinkronkan dari Data Presensi! (Hadir: ${att.hadir}, Sakit: ${att.sakit}, Izin: ${att.izin}, Alpa: ${att.alpa})`);
    } else {
      showToast("Gagal membaca tanggal untuk sinkronisasi Presensi.");
    }
  };
...
const [journalToDelete, setJournalToDelete] = useState<JournalEntry | null>(null);
  ...
  // Sumber Tujuan Pembelajaran untuk form Jurnal: RPM (prioritas) lalu bank Hitung KKTP
  const getSubjectTPOptions = (matpelName: string): string[] => {
    ...
    // 1. Tujuan Pembelajaran terpilih pada Generator RPM (jika mapelnya sama) -> prioritas utama
    if (rpmSource && rpmSource.mapel && rpmSource.tps && rpmSource.tps.length > 0) {
      const rpmName = rpmSource.mapel;
      const isSame = rpmName === matpelName ||
        (matpelName.toLowerCase().includes(rpmName.toLowerCase()) || rpmName.toLowerCase().includes(matpelName.toLowerCase())) ||
        ((matpelName.toLowerCase().includes('pkn') || matpelName.toLowerCase().includes('pancasila')) && (rpmName.toLowerCase().includes('pkn') || rpmName.toLowerCase().includes('pancasila'))) ||
        (matpelName.toLowerCase().includes('ipa') && (rpmName.toLowerCase().includes('ipa') || rpmName.toLowerCase().includes('sains'))) ||
        ((matpelName.toLowerCase().includes('tik') || matpelName.toLowerCase().includes('komputer') || matpelName.toLowerCase().includes('informatika')) && (rpmName.toLowerCase().includes('tik') || rpmName.toLowerCase().includes('komputer') || rpmName.toLowerCase().includes('informatika')));

      if (isSame) {
        return rpmSource.tps;
      }
    }
  ...
  // True jika daftar TP pada matpel ini berasal dari Generator RPM (bukan bank Hitung KKTP)
  const isRpmTpSource = (matpelName: string): boolean => {
    if (!matpelName || !rpmSource || !rpmSource.mapel || !rpmSource.tps || rpmSource.tps.length === 0) return false;
    const rpmName = rpmSource.mapel;
    if (rpmName === matpelName) return true;
    const m = matpelName.toLowerCase();
    const r = rpmName.toLowerCase();
    if (r.includes(m) || m.includes(r)) return true;
    if ((m.includes('pkn') || m.includes('pancasila')) && (r.includes('pkn') || r.includes('pancasila'))) return true;
    if (m.includes('ipa') && (r.includes('ipa') || r.includes('sains'))) return true;
    if ((m.includes('tik') || m.includes('komputer') || m.includes('informatika')) && (r.includes('tik') || r.includes('komputer') || r.includes('informatika'))) return true;
    return false;
  };

  // TP pada Jurnal Mengajar DIKUNCI dari Generator RPM: hanya dapat dicentang melalui menu RPM.
  // Daftar yang tampil & tersimpan diambil dari rpmSource.tps (pilihan guru di Generator RPM).
  const lockedTps = isRpmTpSource(formData.matpel) ? (rpmSource?.tps || []) : [];

  // Baris TP yang sudah tersimpan dalam entri yang sedang diedit tapi tak ada di RPM
  const lockedStoredLines = formData.tujuanPembelajaran
    .split('\n').map((s) => s.trim()).filter(Boolean)
    .filter((t) => !lockedTps.includes(t));
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const isRpm = isRpmTpSource(selectedMatpel);
                  const tps = isRpm ? (rpmSource?.tps || []) : getSubjectTPOptions(selectedMatpel);
                  setFormData(prev => ({
                    ...prev,
                    matpel: selectedMatpel,
                    tujuanPembelajaran: isRpm ? tps.join('\n') : (tps.length > 0 ? tps[0] : prev.tujuanPembelajaran)
                  }));
                }}
                className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 text-xs font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500"
              >
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[10px] font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-md border border-indigo-200 flex items-center gap-1">
                  <Lock className="w-3 h-3 text-indigo-500" />
                  {isRpmTpSource(formData.matpel) ? `RPM: ${lockedTps.length} TP (Terkunci)` : 'TP Dikunci dari RPM'}
                </span>
              </div>

              {/* TP DIKUNCI: hanya dapat dicentang melalui menu Generator RPM */}
              <div className="rounded-xl border border-indigo-200 bg-indigo-50/70 p-3 space-y-2">
                <div className="flex items-start gap-2 text-[11px] text-indigo-800 font-semibold">
                  <Lock className="w-3.5 h-3.5 text-indigo-600 mt-0.5 shrink-0" />
                  <span>TP pada Jurnal ini dikunci dari <strong>Generator RPM</strong>. Untuk mengubah pilihan, centang/batalkan di menu <strong>RPM</strong>.</span>
                </div>

                {lockedTps.length > 0 ? (
                  <div className="space-y-1.5 max-h-52 overflow-y-auto pr-1">
                    {lockedTps.map((tpText, idx) => (
                      <div key={`tp-locked-${idx}`} className="flex items-start gap-2.5 border rounded-lg p-2 bg-indigo-600 border-indigo-700 shadow-xs">
                        <input type="checkbox" checked readOnly disabled className="mt-0.5 w-4 h-4 accent-white shrink-0 cursor-not-allowed" />
                        <span className="text-[11px] leading-snug text-white font-bold">TP {idx + 1}: {tpText}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-[11px] text-slate-600 italic">Belum ada Tujuan Pembelajaran yang dicentang pada Generator RPM untuk mapel ini.</p>
                )}

                {/* Baris TP lama yang tersimpan di entri yang sedang diedit tetapi tidak berasal dari RPM */}
                {lockedStoredLines.length > 0 && (
                  <div className="space-y-1.5">
                    {lockedStoredLines.map((t, idx) => (
                      <div key={`tp-stored-${idx}`} className="flex items-start gap-2.5 border rounded-lg p-2 bg-emerald-600 border-emerald-700 shadow-xs">
                        <input type="checkbox" checked readOnly disabled className="mt-0.5 w-4 h-4 accent-white shrink-0 cursor-not-allowed" />
                        <span className="text-[11px] leading-snug text-white font-bold">TP Kustom (tersimpan): {t}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <p className="text-[10px] text-slate-400 font-semibold mt-1.5">Tujuan Pembelajaran hanya dapat dicentang melalui menu Generator RPM ({isRpmTpSource(formData.matpel) ? `${lockedTps.length} TP terpilih` : 'belum ada TP terpilih'}).</p>
            </div>
          </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b pb-2 border-slate-200 gap-2">
                <div className="flex items-center space-x-1.5 flex-wrap gap-y-1">
                  ...
                  <span className="text-[10px] font-extrabold bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-full border border-emerald-300 flex items-center gap-1">
                    ...
                    Tersinkron dengan Menu Presensi
                  </span>
                </div>
...
<button
                    ...
                    title="Ambil data kehadiran terbaru dari Tab Presensi"
                  >
                    <RefreshCw className="w-3 h-3 text-indigo-600" />
                    <span>Sinkronkan Presensi</span>
                  </button>
              ...
              <div>
                <label className="block text-[11px] font-extrabold text-slate-700 mb-1">
                  Daftar Siswa Tidak Hadir / Catatan (Otomatis Terisi dari Presensi)
                </label>
                <input
                  type="text"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                  <div className="md:col-span-2 space-y-1.5">
                    ...
                    <p className="text-slate-600 whitespace-pre-wrap">
                      {formatNumberedTps(journal.tujuanPembelajaran)}
                    </p>
                    <p className="text-slate-600">
                  ...
                  <table className="w-full border-collapse border border-slate-800 text-xs text-slate-900">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="border border-slate-800 p-2.5 font-medium text-slate-900 whitespace-pre-wrap">{activeJournalToPrint.tujuanPembelajaran}</td>
                      </tr>
                      <tr>
      ...
      <div className="hidden print:block print-jurnal-sheet text-slate-900 font-sans p-2 max-w-3xl mx-auto">
        ...
        <div className="border border-slate-300 rounded-2xl p-6 bg-white space-y-4">
          ...
          <div className="border-b-2 border-slate-900 pb-3 flex items-start justify-between">
            <div className="flex items-start space-x-3">
              {biodata.logo && <img src={biodata.logo} className="print-kop-logo max-h-14 object-contain shrink-0" alt="Logo Sekolah" />}
              <div>
                <h2 className="text-base font-black uppercase tracking-wide text-slate-900">{biodata.namaSekolah || "SD NEGERI BAKUMPUL"}</h2>
            ...
            <table className="w-full border-collapse border border-slate-800 text-xs text-slate-900">
              <tbody>
                ...
                <tr>
                  ...
                  <td className="border border-slate-800 p-2.5 font-medium text-slate-900 align-top whitespace-pre-wrap">{activeJournalToPrint.tujuanPembelajaran}</td>
                </tr>
                <tr>
