...
/** Kunci presensi milik seorang guru; kosong = kunci global (fallback). */
...
/** Baca presensi milik guru; jika belum ada kunci guru, warisi data global lama. */
...
// ============================================================
// Penyimpanan "data kerja" lengkap seorang guru.
// Semua database yang dikerjakan guru (KKTP, Diagnosa, Buku Nilai,
// Bank CP/TP, dst.) disimpan sebagai SATU snapshot per NIP/NIK.
// Dengan begitu, meski guru keluar/masuk aplikasi berkali-kali,
// hasil kerjanya tetap utuh dan otomatis dipulihkan saat login.
// ============================================================

/** Kunci snapshot data kerja milik seorang guru; kosong = kunci global (fallback). */
export const teacherWorkKey = (nip?: string): string => {
  const c = cleanNip(nip);
  return c ? `bakumpul_work_v1_${c}` : 'bakumpul_work_v1';
};

/** Baca snapshot data kerja milik guru (objek generik). */
export const loadTeacherWork = <T,>(nip?: string): T | null => {
  const key = teacherWorkKey(nip);
  try {
    const raw = localStorage.getItem(key);
    if (raw) {
      const parsed = JSON.parse(raw);
  ...
  if (cleanNip(nip)) {
    try {
      const legacy = localStorage.getItem('bakumpul_work_v1');
      if (legacy) {
        const parsed = JSON.parse(legacy);
        if (parsed && typeof parsed === 'object') return parsed as T;
      }
...
/** Simpan snapshot data kerja milik guru. */
export const saveTeacherWork = <T,>(nip: string | undefined, data: T): void => {
  try {
    localStorage.setItem(teacherWorkKey(nip), JSON.stringify(data));
  } catch {
    // abaikan quota error
  }
};
