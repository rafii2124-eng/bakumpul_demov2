...
// ============================================================
// REGISTRASI AKUN GURU lintas perangkat (kunci autentik = NIP/NIK).
// Akun disimpan ke server agar guru tetap dikenali oleh NIP/NIK
// meski login dari perangkat/browser lain — tanpa registrasi ulang.
// ============================================================

export interface GuruAccountRecord {
  npsn: string;
  namaSekolah: string;
  nama: string;
  nip: string;
...
/** Kirim akun guru hasil registrasi ke server (best-effort). */
export const pushGuruRegistration = async (acc: {
  npsn?: string;
  namaSekolah?: string;
  namaGuru?: string;
  nipGuru?: string;
  ...
  try {
    const res = await fetch('/api/guru-register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        npsn: (acc?.npsn || '').trim(),
        namaSekolah: (acc?.namaSekolah || '').trim(),
...
/** Ambil akun guru dari server berdasarkan NIP/NIK (untuk login lintas perangkat). */
export const fetchGuruAccount = async (nip?: string): Promise<GuruAccountRecord | null> => {
  try {
    const c = cleanNip(nip);
    if (!c) return null;
    const res = await fetch(`/api/guru-register?nip=${encodeURIComponent(c)}`);
    if (res.ok) {
      const json = await res.json();
      if (json && json.account) return json.account as GuruAccountRecord;
    }
  } catch {
    // server offline
  }
